...
insensitive R{leftneg=l, leftdollar=ld, middle=m, rightdollar=rd, rightneg=r} = R{leftneg=fmap f l, leftdollar=map(fmap(fmap h)) ld, middle=map(first h<$>) m, rightdollar=map(fmap(fmap h)) rd, rightneg=fmap f r} where
...
match k@R{leftneg=Nothing, leftdollar=[], middle=[], rightdollar = Right(Identity(Ch pats)):xs} stat =  do
...
match k@R{leftneg=Nothing, leftdollar=Right(Identity(Ch pats)):xs} stat =  do
 sensitive <- sensitivity <$> ask
